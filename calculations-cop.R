...
get_cop_results <- function(input_cop) {
  ...
  if(input_cop$Uncertainty_COP == 'EstEff') {
    ...
    validate(
      ...
      need(input_cop$n_obs_cop_ee > (input_cop$n_covariates_cop_ee + 3), 
    ...
    cop_output <- 
      paste0(
        ...
        "which does not depend on the specification of a baseline model, as well as Oster's delta*. ",
        ...
        "the correlation-based delta is ", sprintf("%.3f", raw_calc$`delta_Correlation`),
        ".<br><br>",
        ...
        "<hr>",
        "See Frank et al. (2026) for a description of the method.<br><br>",
        ...
        "<a href='https://doi.org/10.3102/10769986261422704' target='_blank'>",
        "https://doi.org/10.3102/10769986261422704</a>.<br><br>",
    ...
    validate(
      need(is.numeric(input_cop$lower_bnd_cop_ci) &
           ...
           "Did not run! Did you enter numbers for the estimated effect, standard error, number of observations, and number of covariates? Please change any of these that are not a number."),
      need(input_cop$n_obs_cop_ci > (input_cop$n_covariates_cop_ci + 3), 
           "Did not run! There are too few observations relative to the number of observations and covariates. Please specify a less complex model to use KonFound-It."),
